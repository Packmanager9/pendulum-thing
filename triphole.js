...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class NewPendulum{
        constructor(){
            this.color = colors[pendulums.length]
            this.length1 = 141;
            this.length2 = 141;
            this.mass1 = 4;
            this.mass2 = 6;
            ...
            this.x0 = 480
            this.y0 = 480
            this.angle1 = (Math.PI / 2)-(pendulums.length*.0001);
            this.angle2 = Math.PI / 2;
        }
        draw() {
        this.storage1 = -this.gravity * (2 * this.mass1 + this.mass1) * Math.sin(this.angle1);
        this.storage2 = -this.mass2 * this.gravity * Math.sin(this.angle1 - 2 * this.angle2);
        this.storage3 = -2 * Math.sin(this.angle1 - this.angle2) * this.mass1;
        this.storage4 = this.angle2_v * this.angle2_v * this.length2 + this.angle1_v * this.angle1_v * this.length1 * Math.cos(this.angle1 - this.angle2);
        this.den = this.length1 * (2 * this.mass1 + this.mass1 - this.mass1 * Math.cos(2 * this.angle1 - 2 * this.angle2));
        this.angle1_a =  (this.storage1 + this.storage2 + this.storage3 * this.storage4) / this.den;
        this.storage1 = 2 * Math.sin(this.angle1 - this.angle2);
        this.storage2 = this.angle1_v * this.angle1_v * this.length1 * (this.mass1 + this.mass1);
        this.storage3 = this.gravity * (this.mass1 + this.mass1) * Math.cos(this.angle1);
        this.storage4 = this.angle2_v * this.angle2_v * this.length2 * this.mass1 * Math.cos(this.angle1 - this.angle2);
        this.den = this.length2 * (2 * this.mass1 + this.mass1 - this.mass1 * Math.cos(2 * this.angle1 - 2 * this.angle2));
        this.angle2_a =  (this.storage1 *  (this.storage2 + this.storage3 + this.storage4)) / this.den;
        this.x1 = (this.length1 * Math.sin(this.angle1))+this.x0;
        this.y1 = (this.length1 * Math.cos(this.angle1)+this.y0);
        this.x2 = this.x1 + this.length2 * Math.sin(this.angle2);
        this.y2 = this.y1 + this.length2 * Math.cos(this.angle2);
        this.angle1_v += this.angle1_a;
        this.angle2_v += this.angle2_a;
        this.angle1 += this.angle1_v;
        this.angle2 += this.angle2_v;
        let line1 = new Line(this.x1, this.y1, this.x2, this.y2, this.color, 2)
        line1.draw()
        let line2 = new Line(this.x0, this.y0, this.x1, this.y1, this.color, 2)
        line2.draw()
        let point0 = new Circle(this.x0, this.y0, 2, this.color)
        let point1 = new Circle(this.x1, this.y1, this.mass1, this.color)
        let point2 = new Circle(this.x2, this.y2, this.mass2, this.color)
        point0.draw()
        point2.draw()
        point1.draw()
        }
...
for(let t =0;t<colors.length;t++){
    ...
    function main() {
        ...
        gamepadAPI.update() //checks for button presses/stick movement on the connected controller)
                // // game code goes here
        for(let t =0;t<pendulums.length;t++){
                pendulums[t].draw()
        }
    }
})
